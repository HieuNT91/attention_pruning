...
parser.add_argument("--num_heads_in_group", type=int, default=2)
...
if model.distributed_state.is_main_process:
    wandb.login()
    wandb.init(
        project=f"revise_{args.project_id}_{args.dataset}",
        name=f"l_{args.pruned_layer}_nheads_{args.num_heads_in_group}_nsam_{args.num_samples}_model_{args.model_repo.split('/')[-1]}",
        config=vars(args),
    )
...
if args.dataset == 'train':
    prompt_cache_path = f"shapley_prune/tmp/cache/{args.model_repo.split('/')[-1]}_ns{args.num_samples}_sene{args.save_every_n_gens}_bz{args.batch_size}_subsample{args.subsample_size}_cluster{args.n_clusters}/no_prune_{args.dataset}"
else:    
    prompt_cache_path = f"shapley_prune/tmp/cache/{args.model_repo.split('/')[-1]}_ns{args.num_samples}_sene{args.save_every_n_gens}_bz{args.batch_size}/no_prune_{args.dataset}"
